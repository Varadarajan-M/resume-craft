...
const ResumePreviewPanel = ({ className }: ResumePreviewPanelProps) => {
  ...
  const previousTemplateId = useRef(resume?.templateId);
  const timerId = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    // If templateId changes, update debouncedResume immediately
    if (resume?.templateId !== previousTemplateId?.current) {
      setDebouncedResume(resume);
      previousTemplateId.current = resume?.templateId;
    }
